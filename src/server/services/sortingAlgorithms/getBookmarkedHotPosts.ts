...
export const getBookmarkedHotPosts = async ({
  ctx,
  cursor,
  limit,
  skip,
  ...
  const hotPosts = await getBookmarkedHotPostsInDate({
    ctx,
    limit,
    skip,
    userId,
...
const getBookmarkedHotPostsInDate = async ({
  ctx,
  cursorPost,
  date,
  limit,
  ...
  if (hotPosts.length < limit) {
    ...
    const bestPostForYesterday = await getBookmarkedHotPostsInDate({
